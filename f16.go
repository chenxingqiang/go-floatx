...
const (
	...
	F16PrecisionExact F16Precision = iota
...
func F16PrecisionFromfloat32(f32 float32) F16Precision {
	u32 := math.Float32bits(f32)

	if u32 == 0 || u32 == 0x80000000 {
		// +- zero will always be exact conversion
		return F16PrecisionExact
	}
...
func F16Frombits(u16 uint16) Float16 {
	return Float16(u16)
}
...
func F16Fromfloat32(f32 float32) Float16 {
	return Float16(f32bitsToF16bits(math.Float32bits(f32)))
}

// ErrInvalidNaNValue indicates a NaN was not received.
const F16ErrInvalidNaNValue = float16Error("float16: invalid NaN value, expected IEEE 754 NaN")
...
func F16FromNaN32ps(nan float32) (Float16, error) {
	...
	if (exp != 0x7f800000) || (coef == 0) {
		return SNAN, F16ErrInvalidNaNValue
	}
